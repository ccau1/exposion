...
export default (keys: string[], fn: () => void): void => {
  useEffect(() => {
    const keysDown = {};

    const matchKey = (key: string): boolean => {
      return Boolean(keysDown[keyCodes[key]]);
    };

    const matchKeys = (requestedKeys: string[]): boolean => {
      return requestedKeys.every((k) => matchKey(k));
    };
    ...
    document.addEventListener("keydown", keyListeners, { once: false });
    document.addEventListener("keyup", keyListeners, { once: false });
    return () => {
      document.removeEventListener("keydown", keyListeners);
      document.removeEventListener("keyup", keyListeners);
    };
  }, [fn, keys]);
};
